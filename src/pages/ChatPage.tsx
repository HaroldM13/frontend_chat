/**
 * Página principal de JHT Chat.
 *
 * Bug fix mensajes privados:
 *   - useWebSocket solo maneja mensajes RT (tiempo real).
 *   - historial HTTP se carga separado y se combina con useMemo.
 *
 * Responsive:
 *   - Mobile (< md): sidebar oculto por defecto, se abre con hamburguesa.
 *   - Tablet/Desktop (md+): sidebar siempre visible.
 *
 * Funcionalidades:
 *   - Presencia: polling cada 30 s para todos los contactos.
 *   - Marcar leídos: al abrir chat privado y al recibir mensajes del otro.
 *   - Eliminar conversación privada (mantiene el contacto).
 *   - Eliminar / salir de grupo desde la cabecera del chat.
 *   - Modal de perfil: ver info, editar nombre, eliminar cuenta.
 */
...
import { contactosApi, gruposApi, mensajesApi, usuariosApi } from '../services/api'
import { useWebSocket } from '../hooks/useWebSocket'
import type { Contacto, Grupo, ChatActivo, Mensaje, MensajeWS } from '../interfaces'
...
export function ChatPage() {
  ...
  const grupoActivo = chatActivo?.tipo === 'grupo'
  ...
  const confirmarEliminarChat = async () => {
    if (!chatActivo || chatActivo.tipo !== 'privado' || !chatActivo.id || !token) return
    setEliminando(true)
    try {
  ...
  const eliminarGrupoActivo = async () => {
    if (!chatActivo || chatActivo.tipo !== 'grupo' || !chatActivo.id || !token) return
  ...
  return (
    ...
    >
        ...
        {chatActivo ? (
          <>
            <CabeceraChat
              ...
              onEliminarChat={chatActivo.tipo === 'privado' ? () => setModalEliminarChat(true) : undefined}
              onEliminarGrupo={chatActivo.tipo === 'grupo' ? eliminarGrupoActivo : undefined}
              onSalirGrupo={chatActivo.tipo === 'grupo' ? salirGrupoActivo : undefined}
            />
            <AreaMensajes mensajes={todosMensajes} cargando={cargandoHistorial} />
            <InputMensaje onEnviar={enviar} deshabilitado={!conectado} />
          </>
        ) : (
          /* Pantalla de bienvenida */
          <div className="flex-1 flex flex-col items-center justify-center gap-5 p-6 animate-fade-in">
            <button
              onClick={() => setSidebarVisible(true)}
              className="md:hidden absolute top-4 left-4 p-2.5 rounded-xl shadow-card"
              style={{
            ...
            >
              <IconMessageCircle size={48} style={{ color: 'var(--color-accent)' }} />
            </div>
      ...
      {/* Modal Contactos */}
      <Modal titulo={t.contacts.title} abierto={modalContactos} onCerrar={() => setModalContactos(false)}>
        <PanelContactos
          contactos={contactos}
          onActualizar={cargarDatos}
          onIrAlChat={chat => { setChatActivo(chat); setModalContactos(false) }}
          onCerrar={() => setModalContactos(false)}
        />
      </Modal>

      {/* Modal Grupos */}
      <Modal titulo={t.groups.title} abierto={modalGrupos} onCerrar={() => setModalGrupos(false)}>
        <PanelGrupos
          grupos={grupos}
          onActualizar={cargarDatos}
          onIrAlChat={chat => { setChatActivo(chat); setModalGrupos(false) }}
          onCerrar={() => setModalGrupos(false)}
        />
      </Modal>

      {/* Modal Perfil */}
      <Modal titulo={t.profile.title} abierto={modalPerfil} onCerrar={() => setModalPerfil(false)}>
        <ModalPerfil
          onEliminarPerfil={() => { setModalPerfil(false); setModalEliminarPerfil(true) }}
        />
      </Modal>

      {/* Modal Eliminar perfil */}
      <Modal titulo={t.profile.delete} abierto={modalEliminarPerfil} onCerrar={() => setModalEliminarPerfil(false)}>
      ...
      <Modal titulo={t.chat.deleteChat} abierto={modalEliminarChat} onCerrar={() => setModalEliminarChat(false)}>
        <div className="space-y-5">
          <p className="text-sm leading-relaxed" style={{ color: 'var(--color-text-secondary)' }}>
